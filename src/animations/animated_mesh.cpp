#include "animations/animated_mesh.hpp"

AnimatedMesh::AnimatedMesh(std::vector<AnimatedVertex> vertices, std::vector<unsigned int> indices) : vertices(vertices), indices(indices)
{
    setupMesh();
}
...
void AnimatedMesh::setupMesh()
{
    ...
    glVertexAttribIPointer(3, 4, GL_UNSIGNED_INT, sizeof(AnimatedVertex), (void *)offsetof(AnimatedVertex, inBoneIDs));

    // Vertex Bone Weights
    glEnableVertexAttribArray(4);
    glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex), (void *)offsetof(AnimatedVertex, inBoneWeights));
